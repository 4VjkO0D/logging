...
function findFoodMatch(text) {
  ...
  return foods.find(f => lower.includes(f.name.toLowerCase()));
}

function savedFoodsForPrompt() {
  return foods.map(f => ({ name: f.name, calsPerGram: Math.round(f.calsPerGram * 100) / 100 }));
}
...
{"valid": boolean, "name": string, "grams": number, "caloriesTotal": number, "calsPerGram": number, "source": "sparat livsmedel" | "AI-uppskattning"}
...
Användarens sparade livsmedel (använd EXAKT dessa värden om ett omnämnt livsmedel matchar):
${JSON.stringify(savedFoodsForPrompt())}

Annars, ge en rimlig uppskattning av kalorier per gram för den råvara/livsmedel som anges.`;
}
...
Användarens sparade livsmedel (använd EXAKT dessa kcal/gram-värden om ett omnämnt livsmedel matchar något här, gissa då inte annat):
${JSON.stringify(savedFoodsForPrompt())}
...
2. Om mängd (gram) saknas för något som nämnts, fråga efter det innan du anser dig klar.
3. Om tillagningsgrad (kokt/rå/stekt/etc) skulle påverka kalorierna märkbart och inte redan är tydlig eller underförstådd, fråga om det.
4. När du har tillräcklig information för ALLA livsmedel som nämnts i samtalet (namn, gram, källa), svara ENDAST med den bokstavliga texten "KLART:" direkt följt av en JSON-array, utan något annat före eller efter, utan markdown-taggar. Exakt format:
[{"name": string, "grams": number, "caloriesTotal": number, "calsPerGram": number, "source": "sparat livsmedel" | "AI-uppskattning"}]
5. I alla andra fall (du behöver mer info, eller bara konverserar), svara ENDAST med kort, naturlig svensk text — ingen JSON, inget "KLART:"-prefix. Blanda aldrig fritext och JSON i samma svar.`;
}
...
function renderChatMessages() {
  ...
  chatHistory.forEach(m => {
    ...
    const isKlar = m.role === 'assistant' && m.content.trim().startsWith('KLART:');
  ...
  chatPendingDraft.forEach((item, idx) => {
    ...
    row.innerHTML = `
      <label class="field-label">${escapeHtml(item.name)}
        <span class="source-badge ${item.source === 'sparat livsmedel' ? 'saved' : ''}">${escapeHtml(item.source || 'AI-uppskattning')}</span>
      </label>
...
async function sendChatMessage() {
  ...
  try {
    ...
    persistChat();
    renderChatMessages();
    if (raw.trim().startsWith('KLART:')) {
      let jsonPart = raw.trim().slice('KLART:'.length).trim();
      jsonPart = jsonPart.replace(/^```json/i, '').replace(/^```/, '').replace(/```$/, '').trim();
      try {
        const items = JSON.parse(jsonPart);
        chatPendingDraft = items.filter(i => i && i.name && i.grams > 0);
        renderChatPending();
      } catch (e) {
        // Couldn't parse — leave it as a plain message, no pending items shown.
      }
    }
  } catch (err) {
...
function addIngredientRow() {
  ...
  row.querySelector('.ing-ai').addEventListener('click', async () => {
    ...
    const savedMatch = foods.find(f => f.name.toLowerCase() === name.toLowerCase());
    if (savedMatch) {
      densityEl.value = savedMatch.calsPerGram.toFixed(2);
